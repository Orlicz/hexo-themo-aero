...
- ## 高亮
  ...
  #define cts const int&
  #define nt(a) ((a)+n)
  #define int long long
  int n,m,i,j,cnt,clrs,dfns,dfn[maxn],low[maxn],hd[maxn<<1],clr[maxn];
  bool ins[maxn],a,b;
  stack<int> sc;
  struct edge{
    int u, v, nxt;
  }dg[maxn<<1];
  inline void lineto(cts u, cts v) {
    dg[++cnt] = { .u = u,.v = v,.nxt = hd[u] };
    hd[u] = cnt;
  }
  void tarjan(cts u) {
    sc.push(u), ins[u] = 1;
    dfn[u] = low[u] = ++dfns;
    for (auto i = hd[u]; i; i = dg[i].nxt)
      if (const auto& v = dg[i].v;!dfn[v])
  ...
  int main() {
    ...
    while (m--) {
      cin >> i >> a >> j >> b;
      lineto(i + n * (a & 1), j + n * (b ^ 1));
      lineto(j + n * (b & 1), i + n * (a ^ 1));
    }
